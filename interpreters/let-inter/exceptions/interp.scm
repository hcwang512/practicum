...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  ;; Page: 173
  (define value-of/k
    (lambda (exp env cont tr-cont)
      (cases expression exp

        (const-exp (num) (apply-cont cont (num-val num) tr-cont))

        (const-list-exp (nums)
          (apply-cont cont
            (list-val (map num-val nums)) tr-cont))

        (var-exp (var) (apply-cont cont (apply-env env var) tr-cont))

        (diff-exp (exp1 exp2)
          (value-of/k exp1 env
            (diff1-cont exp2 env cont) tr-cont))

        (unop-exp (unop exp1)
          (value-of/k exp1 env
            (unop-arg-cont unop cont) tr-cont))

        (if-exp (exp1 exp2 exp3)
          (value-of/k exp1 env
            (if-test-cont exp2 exp3 env cont) tr-cont))

        (proc-exp (var body)
          (apply-cont cont
            (proc-val
              (procedure var body env)) tr-cont))

        (call-exp (rator rand)
          (value-of/k rator env
            (rator-cont rand env cont) tr-cont))
        ...
        (try-exp (exp1 var handler-exp)
          (value-of/k exp1 env
            cont (try-cont var handler-exp env cont tr-cont)))

        (raise-exp (exp1)
          (value-of/k exp1 env
            (raise1-cont cont) tr-cont)))))

  ;; apply-cont : continuation * expval -> final-expval

  (define apply-cont
    (lambda (cont val tr-cont)
      (cases continuation cont
        (end-cont () val)
        (diff1-cont (exp2 saved-env saved-cont)
          (value-of/k exp2 saved-env (diff2-cont val saved-cont) tr-cont))
        (diff2-cont (val1 saved-cont)
          (let ((n1 (expval->num val1))
                (n2 (expval->num val)))
            (apply-cont saved-cont
              (num-val (- n1 n2)) tr-cont)))
        (unop-arg-cont (unop cont)
          (apply-cont cont
            (apply-unop unop val) tr-cont))
        (if-test-cont (exp2 exp3 env cont)
          (if (expval->bool val)
            (value-of/k exp2 env cont tr-cont)
            (value-of/k exp3 env cont tr-cont)))
        (rator-cont (rand saved-env saved-cont)
          (value-of/k rand saved-env
            (rand-cont val saved-cont) tr-cont))
        (rand-cont (val1 saved-cont)
          (let ((proc (expval->proc val1)))
            (apply-procedure proc val saved-cont tr-cont)))
        ...
        (raise1-cont (saved-cont)
          ...
          (cases continuation tr-cont
            (try-cont (var handler-exp saved-env saved-cont saved-try)
              (value-of/k handler-exp
                (extend-env var val saved-env) saved-cont saved-try))
            (else (eopl:error 'apply-cont "not try-cont ~s" tr-cont))))
          ;(apply-handler val saved-cont try-cont))
        )))
      
  ;; apply-handler : ExpVal * Cont -> FinalAnswer
  ;(define apply-handler
    ;(lambda (val cont)
      ;(eopl:printf "search handler ~s" val)
      ;(cases continuation cont
        ;;; interesting cases
